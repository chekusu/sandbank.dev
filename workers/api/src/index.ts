...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    return proxyToNode(request, url, selectedNode, env)
  },
}

async function proxyToNode(request: Request, originalUrl: URL, node: NodeTarget, env: Env): Promise<Response> {
  ...
  try {
    return markResponse(await fetch(buildNodeRequest(request, originalUrl, node.origin)), node, false)
  } catch (err) {
    ...
    try {
      return markResponse(await fetch(buildNodeRequest(fallbackSource, originalUrl, fallbackOrigin)), fallbackNode, true)
    } catch (fallbackErr) {
...
function buildNodeRequest(request: Request, originalUrl: URL, origin: string): Request {
